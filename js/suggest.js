function start(){
	var oTextbox = new suggestControl(document.getElementById("txtBox"), new suggestionProvider());
}
...
suggestControl.prototype.xmlToArrayId = function(aSuggestions){
	...
	for(i=0; i<resultsNodeXml.length; i++){
		idArray[i] = resultsNodeXml[i].getAttribute("isbn");
	}
	return idArray;
};
...
suggestControl.prototype.showSuggestions = function(bSuggestions, suggestionId){
	...
	for(var i=0; i < bSuggestions.length; i++){
		...
		aLink.href = 'http://www.stephenknutter.com/book_db/display_book.php?isbn='+suggestionId[i];
...
suggestionProvider.prototype.requestSuggestions = function(oAutoSuggestControl, bTypeAhead){
	...
	var oData = "?keyword=" + encodeURIComponent(oAutoSuggestControl.userText);
